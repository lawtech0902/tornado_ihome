...
class AreaInfoHandler(BaseHandler):
    ...
    def get(self):
        ...
        if areas:
            ...
            return self.write('{"errno": 0, "errmsg":"OK", "data":{}}'.format(areas))
        ...
        try:
            ...
            logging.error(e)
            return self.write({"errno": RET.DBERR, "errmsg": "获取数据错误"})

        json_area_data = json.dumps(areas)
        resp = '{"errno":"0", "errmsg":"OK", "data":{}}'.format(json_area_data)
        try:
...
class HouseInfoHandler(BaseHandler):
    ...
    def get(self):
        ...
        # 先从redis缓存中获取信息
        try:
            res = self.redis.get("house_info_{}".format(house_id))
        except Exception as e:
            logging.error(e)
            res = None
        if res:
            logging.info("hit house info redis")
            return self.write('{"errno":"0", "errmsg":"OK", "user_id":{}, "house":{}}'.format(user_id, res))
        ...
        house = {
            ...
            "user_avatar": image_url_prefix + res["up_avatar"]
        }
        ...
        if res:
            for image in res:
                house["image_urls"].append(image_url_prefix + image["hi_url"])

        # 查询房屋的基本设施
        sql = "select hf_facility from ih_house_facility where hf_house_id=%s"
        try:
            res = self.db.query(sql, house_id)
        except Exception as e:
            logging.error(e)
        ...
        try:
            res = self.db.query(sql, house_id, constants.HOUSE_DETAIL_COMMENT_DISPLAY_COUNTS)
        except Exception as e:
            logging.error(e)
            res = None
        ...
        try:
            self.redis.setex("house_info_{}".format(house_id), constants.HOUSE_DETAIL_REDIS_EXPIRE_SECOND, json_house)
        except Exception as e:
            logging.error(e)
        resp = '{"errno":"0", "errmsg":"OK","user_id":{}, "house":{}}'.format(user_id, json_house)
        self.write(resp)
    ...
    def post(self):
        ...
        if facility:
            ...
            try:
                ...
                logging.error(e)
                try:
                    self.db.execute("delete from ih_house_info where hi_house_id={}".format(house_id))
                except Exception as e:
                    logging.error(e)
